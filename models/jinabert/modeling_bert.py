...
try:
    ...
    def scaled_dot_product_attention_flashV2(q, k, v, bsz, q_len, num_heads, head_dim, attention_mask=None,alibi=None):
        #print("alibi_slope ==>", alibi)
        if attention_mask is None:
            output = flash_attn_func(q, k, v, 0.0, softmax_scale=None, causal=False, alibi_slopes=alibi).reshape(
                bsz, q_len, -1
            )
        else:
            #print(attention_mask)
            #q_attention_mask = torch.fill(attention_mask[:, -q_len:],True)
            q_attention_mask = attention_mask[:, -q_len:]
...
class JinaBertSelfAttention(nn.Module):
    def __init__(self, config: JinaBertConfig, position_embedding_type=None):
        ...
        self.config.attn_implementation = config.attn_implementation
    ...
    def forward(
        ...
        if self.config.attn_implementation == "flashV2":
            assert past_key_value is None
            assert encoder_hidden_states is  None
            #print(bias.shape)
            #assert len(bias.shape) == 1
            alibi_slope =  bias[0,:,1,0].float().contiguous()
            b, s, _ = hidden_states.shape
            q = self.query(hidden_states).view(b, s, self.num_attention_heads, self.attention_head_size)
            k =   self.key(hidden_states).view(b, s, self.num_attention_heads, self.attention_head_size)
            v = self.value(hidden_states).view(b, s, self.num_attention_heads, self.attention_head_size)
            output2 = scaled_dot_product_attention_flashV2(q, k, v, 
                                                            b, s, self.num_attention_heads, self.attention_head_size, 
                                                            attention_mask= attention_mask[:,0,0]==0, 
                                                            alibi=-repeat(alibi_slope,"h -> b h",b=b).contiguous()
                                                            )
        ...
        if self.config.attn_implementation == 'torch' and scaled_dot_product_attention is not None:
            ...
            output1 = attn.view(b, s, self.all_head_size)
...
def build_dynamically_alibi_tensor(num_heads, max_pos, origin_max_pos) -> torch.Tensor:
    """Psuedo code for Dynamic NTK-ALiBi."""

    # dynamic ntk factor according to actual sequence length
    a0 = 1.0
    a = a0 * torch.tensor(max_pos) / origin_max_pos  # [batch, 1]
...
class JinaBertEncoder(nn.Module):
    ...
    def rebuild_alibi_tensor(
        ...
        rank = os.environ['RANK'] if 'RANK' in os.environ else 0

        if (self.config.alibi_scaling is None or self.config.alibi_scaling==1):
            return self.old_rebuild_alibi_tensor(size,device)
        if local_rank+rank == 0:print(f""" 
              Warning: you are using a alibi_scaling={self.config.alibi_scaling}>1 model, 
                      you set your max_position_embeddings={self.config.max_position_embeddings} 
                      this mean you want to explatation your model performance to ({self.config.alibi_scaling*self.config.max_position_embeddings}).
                      Do not forget to set your max_position_embeddings={self.config.alibi_scaling*self.config.max_position_embeddings} directly in your config file.
              """)
        n_heads = self.num_attention_heads
        context_position = torch.arange(size, device=device)[:, None]
        memory_position = torch.arange(size, device=device)[None, :]
        ...
        # [n_heads, max_token_length, max_token_length]
        relative_position = relative_position.unsqueeze(0).expand(n_heads, -1, -1)
        slopes = torch.Tensor(build_dynamically_alibi_tensor(n_heads, self.config.alibi_scaling*self.config.max_position_embeddings, self.config.max_position_embeddings)).to(device) * -1
...
class JinaBertModel(JinaBertPreTrainedModel):
    ...
    def __init__(self, config: JinaBertConfig, add_pooling_layer=True):
        ...
        self.pooler = JinaBertPooler(config) if add_pooling_layer and not config.embedding_model else None
