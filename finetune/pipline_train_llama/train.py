...
from transpeeder.models.patching import (
    ...
    addbias,
    replace_state_load_to_skip_bias
)
...
def main():
    ...
    # pipeline model
    model = get_model(model_config, args, activation_checkpointing_config, 
                      partition_method="type:ParallelTransformerLayerPipe",
                      bias = True)
    optimizer_grouped_parameters = [
            {
                "params": [p for name,p in model.named_parameters() if any(word in name for word in ['mlp','34'])],
                "lr": 1e-4
            },
            {
                "params": [p for name,p in model.named_parameters() if not any(word in name for word in ['mlp','34'])],
                "lr": 0
            },
        ]
    ...
    #     if any(word in name for word in ['mlp','33','33']):
