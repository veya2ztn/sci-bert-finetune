...
class ParallelTransformerLayerPipe(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, activation_checkpointing=False, bias = False):
        super().__init__(config)
        self.activation_checkpointing = activation_checkpointing
        if bias:
            self.self_attn.o_proj.bias    = torch.nn.Parameter(torch.zeros_like(self.self_attn.o_proj.weight[:,0]))
            self.mlp.down_proj.bias       = torch.nn.Parameter(torch.zeros_like(self.mlp.down_proj.weight[:,0]))
...
def get_model(model_config: LlamaConfig, args, activation_checkpointing_config=None, bias=False, **kwargs):
        ...
        def __init__(self, model_config, **kwargs):
            ...
            super().__init__(
                layers=[
                  ...
                  *[LayerSpec(ParallelTransformerLayerPipe, model_config, bias=bias) for _ in range(model_config.num_hidden_layers)],
                    LayerSpec(LayerNormPipe, model_config.hidden_size, model_config.rms_norm_eps),
                    LayerSpec(LMLayerPipe, model_config.hidden_size, model_config.vocab_size, bias=bias),
                ],
