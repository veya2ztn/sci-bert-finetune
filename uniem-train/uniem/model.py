...
from uniem.criteria import (
    ...
    PairInBatchNegSoftmaxContrastLossLabel
)
...
class PoolingStrategy(str, Enum):
    ...
    llama_last_weighted = 'llamalast_weighted'
...
class LlamaLastWeightedEmbedder(UniemEmbedder):
    pooling_strategy: ClassVar[PoolingStrategy] = PoolingStrategy.llama_last_weighted
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        embeddings = self.encoder(input_ids).hidden_states[-1]
...
class BertEmbedder(torch.nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor | None = None):
        if attention_mask is None:
            attention_mask = create_attention_mask_from_input_ids(input_ids, self.pad_token_id)
        embeddings = self.encoder(input_ids =  input_ids, attention_mask = attention_mask)[0]
        embeddings = self.encoder.mean_pooling(embeddings, attention_mask)
        return embeddings
...
def create_uniem_embedder(
    ...
    pooling_strategy: PoolingStrategy | str = PoolingStrategy.last_mean,**kargs
):
    pretrained_model = load_hf_pretrained_model(model_name_or_path, model_class=model_class,**kargs)
...
class EmbedderForPairInBatchNegTrainWithLabel(EmbedderForTrain):
    def __init__(
        self,
        embedder: Embedder,
        temperature: float = 0.05,
        ...
        criterion      = PairInBatchNegSoftmaxContrastLossLabel(temperature)
        super().__init__(embedder, criterion)
