...
class PairInBatchNegSoftmaxContrastLossLabel(ContrastLoss):
    """
    This is design for softlabel of ground truth.
    The original labels is one-hot vector, but it may be a softlabel for a ranked system in the future.
    ...
    def forward(
        ...
        text_pos_embeddings: torch.Tensor, # (M, D)
        labels: torch.Tensor,
        output_prob: bool = False,
        temperature: float = None
    ) -> torch.Tensor:
        
        if output_prob and text_embeddings.shape[1] == 1 and text_pos_embeddings.shape[1] == 1:  
            text_embeddings = text_embeddings[:,0]
            text_pos_embeddings = text_pos_embeddings[:,0]
            comparison_matrix = text_embeddings.unsqueeze(1) == text_pos_embeddings.unsqueeze(0)
            locations = torch.where(comparison_matrix)[1]
            return locations
        sim_matrix = torch.cosine_similarity(
            text_embeddings.unsqueeze(1),
            text_pos_embeddings.unsqueeze(0),
