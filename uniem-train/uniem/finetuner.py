...
class FineTuner:
    ...
    def get_criterion(self, temperature):
        assert self.record_type == RecordType.PAIR
        loss_type  = InBatchNegLossType.softmax
    ...
    def run(
        ...
        model = self.create_embedder_for_train(temperature=temperature) ## <-- add loss
        #criterion = self.get_criterion(temperature=temperature)
        model = training_strategy.apply_model(model)
        # Optimizer & LRScheduler
        lr = lr or suggest_lr(model)  # type: ignore
