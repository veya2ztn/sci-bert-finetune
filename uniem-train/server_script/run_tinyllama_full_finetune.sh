#!/bin/sh
#SBATCH -J SFTTL      # 作业在调度系统中的作业名为myFirstJob;
#SBATCH -o log/%j-SFTTL.out  # 脚本执行的输出将被保存在20210827-%j.out文件下，%j表示作业号;
#SBATCH -e log/%j-SFTTL.out  # 脚本执行的输出将被保存在20210827-%j.out文件下，%j表示作业号;
...
--bf16 True --output_dir ./checkpoints/full_finetune_online_negitive --num_train_epochs 3 \
--per_device_train_batch_size 60 --gradient_accumulation_steps 60 \
--evaluation_strategy "no" \
--save_strategy "steps" --save_steps 100 --save_total_limit 3 --learning_rate 1e-5 --weight_decay 0. --warmup_ratio 0.03 \
--lr_scheduler_type "cosine" --logging_steps 1 --tf32 False --q_lora False --model_max_length 8000 --ddp_find_unused_parameters False \
--dispatch_batches False --negative_sampler_num -1 --flash_attn --full_finetune \
