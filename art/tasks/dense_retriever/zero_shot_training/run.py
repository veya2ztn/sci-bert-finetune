...
def zero_shot_retriever(dataset_cls):
    ...
    def model_provider():
        """Build the model."""
        args = get_args()
        print_rank_0('building PreComputedEvidenceDocsRetriever model for {} ...'.format(args.task))
        evidence_retriever = PreComputedEvidenceDocsRetriever()
        print_rank_0('building PreComputedEvidenceDocsRetriever model, done!')
        if args.task == 'Train_A_Judger':
            print_rank_0('building JRT model for {} ...'.format(args.task))
            model              = JRTModel(evidence_retriever)
            print_rank_0('building JRT model, done!')
        else:  
            print_rank_0('building ART model for {} ...'.format(args.task))
            model              = ARTModel(evidence_retriever)
            print_rank_0('building ART model, done!')
        return model
...
def main():
    ...
    elif args.task == "Train_A_Judger":
        from tasks.dense_retriever.zero_shot_training.train_data_utils import WhatIsQuestionDataset as dataset_cls
    else:
