        ...
        if self._word_embeddings_key in state_dict:
            ...
            for key in state_dict.keys():
                if 'word_embeddings' in key:
                    loaded_weight = state_dict[key] #should be a tensor (30522, 768)
                    # lets pad it to correct size 
                    target_vocab_size = self.word_embeddings.num_embeddings
                    if len(loaded_weight) != target_vocab_size:
                        print('***WARNING*** expected tokentype embeddings in the '
                            f'checkpoint should be {target_vocab_size} find {len(loaded_weight) }'
                            'we will padding', flush=True)
                        loaded_weight = torch.nn.functional.pad(loaded_weight,(0,0,0,target_vocab_size - len(loaded_weight)))
                        assert (loaded_weight[-1]==0).all()
                    state_dict_[key.split('word_embeddings.')[1]] = loaded_weight
...
class TransformerLanguageModel(MegatronModule):
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        state_dict_ = dict([(key.replace('.attention.', '.self_attention.'),val) for key,val in state_dict_.items()])
