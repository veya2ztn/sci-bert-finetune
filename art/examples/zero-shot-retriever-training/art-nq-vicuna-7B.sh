...
EVIDENCE_DATA_PATH="${DATA_DIR}/wikipedia-split/psgs_w100.hdf5"

# Please specify the model name from Huggingface models
CROSS_ATTENTION_SCORER="pretrain_weights/vicuna-7b-v1.1"
...
PRETRAINED_RETRIEVER_CHECKPOINT="/mnt/data/ai4earth/zhangtianning/LLM/checkpoints/mss-retriever-${RETRIEVER_CONFIG}"
CHECKPOINT_PATH="/mnt/data/ai4earth/zhangtianning/LLM/checkpoints/nq-mss-${RETRIEVER_CONFIG}-init"
EMBEDDING_PATH="/mnt/data/ai4earth/zhangtianning/LLM/embedding-path/art-finetuning-embedding-vicuna/mss-retriever-base.pkl"
...
OPTIONS=" \
          ...
          --indexed-evidence-t0-tokenized-data-path ${DATA_DIR}/evidence-wikipedia-indexed-mmap/vicuna/wikipedia-evidence-vicuna_text_document \
          --indexed-title-t0-tokenized-data-path ${DATA_DIR}/evidence-wikipedia-indexed-mmap/vicuna/wikipedia-evidence-vicuna_title_document \
          --save-interval 500 \
...
COMMAND="WORLD_SIZE=${WORLD_SIZE} ${DISTRIBUTED_ARGS} tasks/run.py ${OPTIONS} ${CONFIG_ARGS}"
echo "$COMMAND"
exit
eval "$COMMAND"
exit
